...
PDF attachments open directly in a new tab (native OS/browser PDF handling) rather than rendering inside the app. That's a deliberate step back from a fancier approach: PDFs used to render fully in-app, page by page onto a `<canvas>`, via a vendored copy of [pdf.js](https://mozilla.github.io/pdf.js/). That was itself the fix for two earlier, lighter approaches that both hit real platform limitations — an `<iframe>` embed doesn't render inline on Android Chrome, and a real same-window navigation to the PDF's `blob:` URL takes over the whole installed-PWA window with no reliable way back short of the OS back gesture. pdf.js sidestepped both, but real-world ticket PDFs kept failing to parse there — and not just ones received via share; the same files failed when added through the ordinary "Choose Files" picker too, ruling out a share-pipeline bug — while opening the exact same file directly always worked. So the in-app renderer (and the vendored pdf.js dependency) was dropped: a PDF's slide in the attachment carousel is now just an icon, filename, and an **Open PDF** button that opens it in a new tab. A new tab keeps the "don't lose the app" property that ruled out the old same-window navigation — closing it leaves Showtime exactly where it was.

Some vendor apps' "share ticket" action and Google Drive's "share link" (vs. "send a copy") can hand the share target an empty or near-empty file that looks attached but can never actually open — those are caught and rejected right when the share arrives (anything under 100 bytes), with a message pointing at the real fix: share/save a copy, not a link. Images inside the carousel viewer are `pointer-events: none`, so a long-press routes entirely to the card underneath (opening our own viewer) instead of also triggering Android Chrome's native "Copy/Download/Share image" menu on the `<img>` itself.
...
4. If there's at least one existing ticket, it always asks first: create a new ticket, or add this to one you already saved (a picker lists them, soonest first, past events left off — nothing to attach a new ticket to there) — regardless of whether the share included a file, text, or both. With no tickets yet, it skips straight to a new, pre-filled **Add event** form.
