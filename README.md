...
If a PDF genuinely won't parse, "Couldn't display this PDF" now comes with a **Try opening it directly** link (the raw file in a new tab) rather than a dead end, and the real pdf.js error is logged to the console instead of swallowed. Some vendor apps' "share ticket" action and Google Drive's "share link" (vs. "send a copy") can hand the share target an empty or near-empty file — that looked identical to a real PDF failure from inside the viewer, so shared files under 100 bytes are now caught and rejected at share time instead, with a message pointing at the actual fix (share a saved copy, not a link).
