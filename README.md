...
Tap **+** to add a ticket: show/event name, venue, date & time (24-hour, quarter-hour increments), price paid, seat/section, where you bought it, confirmation number, and any number of photos/PDFs of the actual tickets (useful when an order has multiple physical tickets, each with its own QR code — attach them all to the one event). Every ticket card shows a consistent purple ticket-icon glyph (not a preview of the attached file — kept deliberately uniform rather than a per-file thumbnail), plus a small count badge when more than one file is attached. On a ticket card: a short tap opens **Edit**; press and hold opens the attached file(s) in a full-size viewer — swipeable left/right between them with page dots when there's more than one, native horizontal scroll-snap rather than custom gesture code — without going through Edit first; a right swipe past ~40% of the card's width deletes it, with a 3-second **Undo** bar before the deletion is actually committed. The same swipeable viewer opens from tapping a thumbnail in the Edit form's file list too, starting on whichever file was tapped.
